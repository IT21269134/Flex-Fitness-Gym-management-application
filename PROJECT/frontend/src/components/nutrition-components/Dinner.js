...
import AddNewDiary from "./AddNewDinner";
...
function Dinner() {
  ...
  const handleUpdate = () => {
    form.validateFields().then((values) => {
      ...
      axios
        .put(`http://localhost:4000/dinner/counterupdate/${id}`, updateData)
        .then((res) => {
  ...
  const handleDeleteConfirmation = async () => {
    try {
      await axios.delete(`http://localhost:4000/dinner/delete/${id}`);
  ...
  return (
    <div className="category-body">
      ...
      <div className="view-analysis-btn-div">
        <Button as={Link} to="/dinner/viewanalysis">
          View Analysis
        </Button>
      </div>
